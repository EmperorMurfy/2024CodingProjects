...
// Because of it's nature as a red square and the only red object, it is worth more to eliminate but moves faster, and there is only a certain amount of it and they move extremely quickly. 
...
public class RedSquare extends Shape{
	...
	public RedSquare(int x_coordinate, int y_coordinate, Color color, int vertical, int horizontal,  int height){
	...
	// method: move
	// description: This method moves the x and y of the shape when called on, and checks if it hits one of the borders (width and height) and reverses it's course (like a bounce) and moves * 2 average speed
	public void move() { //width: 800 height: 480

		setY(getY() + (2*getVertical()));
		if (getY() > 480 || getY() <= 0) {
			setVertical(-getVertical());
		}

		setX(getX() + (2*getHorizontal()));
		if (getX() > 800 || getX() <= 0) {
